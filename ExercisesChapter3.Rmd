...
ggplot(data = Galton, mapping = aes(x=height,y=father)) + 
  geom_point()+facet_wrap(~ sex, ncol = 1)
```
3. Add regression lines to all of your facets.
```{r}
ggplot(data = Galton, mapping = aes(x=height,y=father)) + 
  geom_point()+facet_wrap(~ sex, ncol = 1) +geom_smooth(method="lm", se=0)
```
...
```{r}
ggplot(data = Marriage, mapping= aes(x=age, y=prevcount)) + geom_point(mapping = aes(color= person))
```

2. Indentify each of the visual cues that you are using. 

Visual cue of position is used to know the age and prevcount. Visual cue of color is used to specify person (bride or groom).
...
```{r}
ggplot(data = Marriage, mapping = aes(x = age, y = prevcount, shape=race)) +geom_point(mapping = aes(color = person))+facet_wrap(~ prevconc, ncol = 1)
```
...
```{r}
ggplot(data = MLB_teams, mapping = aes(x = attendance, y =L, size=W, shape=lgID)) +geom_point(mapping = aes(color = name))+facet_wrap(~ yearID, ncol = 1)
```
